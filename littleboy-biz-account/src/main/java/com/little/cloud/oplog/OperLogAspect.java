...
//  private ExceptionLogService exceptionLogService;
   ...
   */
  @Pointcut("execution(* com.little.cloud.controller..*.*(..))")
  ...
  public void saveOperLog(JoinPoint joinPoint, Object keys) {
    ...
    String opModule = opLog.opModule();
    String opType = opLog.opType();
    String opDesc = opLog.opDesc();
    ...
    if ("POST".equalsIgnoreCase(requestMethod)) {
      ...
      // 将参数所在的数组转换成json
//      params = JsonUtils.stringify(reqMap);
    }

    //请求路径
    String url = request.getRequestURI();

    log.info("{} > {} > {} > {}", opModule, opType, opDesc, methodName);
    log.info("{} > {}", url, params);
